...
class lle_net(nn.Module):
    ...
    def __init__(self):
        ...
        # 设置迭代次数
        self.ite_number = 2
        self.output_dim = self.ite_number * 6
    ...
    def forward(self, x):
        def LLE_LUT(origin_img, t, b):
            ...
            print("input image.shape is", origin_img.shape)
        ...
        def lut_init(x_origin, t, b):
            # print("input image.shape is", x_origin.shape)
        ...
        t1, b1, t2, b2 = torch.split(x1, 3, dim=1)
        ...
        return enhance_image_1, enhance_image_2, x1   # enhance_image_3, enhance_image_4,
...
if __name__ == "__main__":
    ...
    print(model)
    images = torch.randint(0, 256, (2, 3, 8, 8), dtype=torch.float32, requires_grad=True)
    print("==============images=========")
    print(images)
    input = torch.ones((4, 3, 128, 128))
    input = images / 255.0
    input = input.cuda()
    # le1, le2, le3, le4, params = model(input)

    le1, le2, params = model(input)
    print("output.shape is ", le2.shape)
